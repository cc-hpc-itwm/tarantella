...
_PREPROCESS_NUM_THREADS = 8 # instead of the TF default tf.data.experimental.AUTOTUNE
_PREFETCH_NBATCHES = 2  # should be ~ the number of devices
...
def process_record_dataset(dataset,
  ...
  Returns:
    Dataset of (image, label) pairs ready for iteration for the current rank.
   """
  
  # Prefetches a micro-batch at a time to smooth out the time taken to load input
  ...
  if is_training:
    ...
    # Same approach in Goyal et al., "Accurate, Large Minibatch SGD: 
  ...
  dataset = dataset.map(
      ...
      num_parallel_calls=_PREPROCESS_NUM_THREADS)
  ...
  dataset = dataset.prefetch(buffer_size=_PREFETCH_NBATCHES)
  return dataset
...
def input_fn(dataset_type,
  ...
  dataset = dataset.interleave(
      ...
      cycle_length=_PREPROCESS_NUM_THREADS,
      num_parallel_calls=_PREPROCESS_NUM_THREADS)
